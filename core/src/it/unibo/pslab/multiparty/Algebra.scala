...
import MultiPartyLanguage.{MultiParty, on, PlacedKind}
...
enum MultiPartyGrammar[T]:
  ...
  case CommPerPeer[V, From <: TieToMultiple[To], To <: Peer](ps: LocalPeer[?], value: PerPeer[V] on From)
      extends MultiPartyGrammar[V on To]
  case Comm[V, From <: TieTo[To], To <: Peer](ps: LocalPeer[?], value: V on From)
